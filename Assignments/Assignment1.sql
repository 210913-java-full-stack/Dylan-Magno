...
###########################################################
DROP DATABASE IF EXISTS Dylan_db;
CREATE DATABASE Dylan_db;
...
SELECT c.name AS Smith
FROM customers c 
WHERE name LIKE '%Smith';

# Get the total balance of all accounts held by the Smiths.
SELECT c.name AS Smith, SUM(a.balance) AS Balance 
FROM customers c 
JOIN accounts a ON  
...
SELECT c.name AS Name, a.balance AS balance, ad.address AS Address
FROM customers c 
JOIN address ad ON c.address_id = ad.address_id
JOIN accounts_customers ac ON c.customer_id = ac.customer_id
JOIN accounts a ON ac.account_id = a.account_id
